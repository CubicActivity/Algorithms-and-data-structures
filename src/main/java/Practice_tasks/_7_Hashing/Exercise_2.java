...
class Ticket{
    ...
    @Override
    public String toString(){
        return ime + " "+prezime+" with salary "+budget+" from address "+ipAddress+" who spent " + price;
    }
        ...
        for(int i=0;i<n;i++){
            ...
            if(ticket.budget>=ticket.price) {
                ...
                if(buckets.search(ticket.city)==null){
                    ...
                    if(buckets.search(ticket.city).element.value.price<price){
