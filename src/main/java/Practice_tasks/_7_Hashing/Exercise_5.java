...
class Ticket{
    ...
    public Ticket(String name, String surname, int budget, String ipaddr, String time, String city, int price) {
        this.name = name;
    ...
    @Override
    public String toString(){
        return name+" "+surname+" with salary "+budget+" from address "+ipaddr+" who spent "+price;
    }
...
class prog{
    public static void main(String [] args){
        ...
        OBHT<String, Integer> countOfNODES = new OBHT<>(k+1);
        CBHT<String, Ticket> nerds = new CBHT<>(k+1);
        ...
        for(int i=0;i<k;i++){
            ...
            if(budget>=price){
                ...
                String realip= ip[0]+"."+ip[1]+"."+ip[2];


                if(countOfNODES.search(realip)==-1){
                    countOfNODES.insert(realip,1);
                }else{
                    countOfNODES.getBucket(countOfNODES.search(realip)).value++;
                }


                if(nerds.search(realip)==null){
                    nerds.insert(realip,ticket);
                }else{


                    if(price>nerds.search(realip).element.value.price){
                        nerds.search(realip).element.value=ticket;
                    }
        ...
        for(int i=0;i<n;i++){
            ...
            System.out.println(countOfNODES.getBucket(countOfNODES.search(realip)).value);
            System.out.println("The user who spent the most from that network is:");
            System.out.println(nerds.search(realip).element.value.toString());
            System.out.println();
