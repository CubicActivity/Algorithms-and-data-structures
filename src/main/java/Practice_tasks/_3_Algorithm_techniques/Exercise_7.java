...
public class Exercise_7 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            nizaProdukt[i] = niza[i];
        }

        for (int i = 1; i < n; i++){
            for (int j = 0; j < i; j++){
                if (niza[i] > niza[j] && nizaProdukt[i] < (nizaProdukt[j] * niza[i])) {
                    nizaProdukt[i] = nizaProdukt[j] * niza[i];
                }
            }
        }
        ...
        int max=0;
        for (int i = 0; i < n; i++){
            max=Math.max(max, nizaProdukt[i]);
        }
