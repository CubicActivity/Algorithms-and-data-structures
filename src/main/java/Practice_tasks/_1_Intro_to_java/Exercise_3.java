...
class SalesPerson {
    ...
    public int sumSales(SalesPerson sp){
        ...
        for(int i=0;i<quarters.length;i++) {
            ...
            for (int j = 0; j < quarters[i].getRevenues().length; j++) {
                total += quarters[i].getRevenues()[j];
    ...
    public void print(){
        ...
        for(int i=0;i<quarters.length;i++) {
            int loopable=0;
            for (int j = 0; j < quarters[i].getRevenues().length; j++) {
                loopable+=quarters[i].getRevenues()[j];
            }
            System.out.print(loopable+"   ");
...
public class Main {
    ...
    public static SalesPerson salesChampion(SalesPerson [] arr)
    {
        ...
        for(int i=0;i<arr.length;i++){
            for(int j=0;j<arr[i].getQuarters().length;j++){
                if(arr[i].sumSales(arr[i])>largest){
                    largest=arr[i].sumSales(arr[i]);
                    index=i;
                }
            }
    ...
    public static void main(String[] args) {
        ...
        for(int i=0;i<n;i++)
        {
            ...
            QuarterlySales[] prodazhbi = new QuarterlySales[4];
            for(int k=0;k<4;k++){


                int numSales = input.nextInt();
                input.nextLine();
                int [] prihod = new int[numSales];
                brProdazbi+=numSales;

                for(int j=0;j<numSales;j++){
                    prihod[j]=input.nextInt();
                }
                input.nextLine();

                prodazhbi[k] = new QuarterlySales(numSales,prihod,i);

                //your code goes here

            }
            arr[i]=new SalesPerson(name,prodazhbi);
