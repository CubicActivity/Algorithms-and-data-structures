...
public class Exercise_6 {
    public static void main(String[] args) {
        ...
        //must add sc.nextline after inputting int with scanner
        SLLNode<String>tmp2 = list.getFirst();
        SLLNode<String>tmp = list.getFirst();
        int requiredLength=sc.nextInt();

        System.out.println(list.toString());

        SLLNode<String>lst=null;

        for(int i=0;i<n;i++){
            if(tmp.element.length()==requiredLength){
                lst=tmp;
//                System.out.println("DELETING "+tmp.element);

            }
            if(tmp.succ!=null)tmp=tmp.succ; else break;
        }
        if(lst!=null) {
            list.delete(lst);
            list.insertFirst(lst.element);
