...
class BinarySearchTree<E extends Comparable<E>> {
     ...
     * @param t the node that roots the tree.
     */
    public int printTree(BNode<Integer> t, int k) {
        int counter =0;
        if (t != null) {
            if(t.info<k)counter++;

            counter+= printTree(t.left,k);
            counter+= printTree(t.right,k);
        }
        return counter;
    }
    ...
    public static void main(String[] args){
        ...
        for(int i=0;i<n+q;i++){
            ...
            if(parts[0].equals("insert")){
                ...
                System.out.println(bst.printTree(bst.getRoot(), Integer.parseInt(parts[1]) ));
            }
        }
