...
In the following task, you need to build a binary tree with N nodes, for which you will need to answer Q questions of the type "what is the sum of degrees in the subtree of a selected node"..
...
class BTree<E> {
    ...
    public int inorderR(BNode<E> n) {
        ...
        if(n!=null) {
            ...
            counter += inorderR(n.left);
            counter += inorderR(n.right);
        }
        return counter;
    }
    ...
    public static void main(String[] args) {
        ...
        for(int i=0;i<n+k;i++){
            ...
            if(parts[0].equals("root")){
                ...
                int where = parts[3].equals("LEFT")? 1 : 2;
                BNode <String> added = tree.addChild(map.get(parts[1]),where,parts[2]);
                map.put(parts[2],added);
            }else if(parts[0].equals("ask")){
                //колку јазли со точно две деца има во поддрвото на избран јазол“.
                System.out.println(tree.inorderR(map.get(parts[1])));
            }
